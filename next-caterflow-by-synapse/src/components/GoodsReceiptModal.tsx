...
import { FiCheck, FiSave, FiX, FiCheckCircle, FiFileText, FiDollarSign, FiPackage, FiGrid, FiMapPin, FiAlertCircle } from 'react-icons/fi';
...
export default function GoodsReceiptModal({
    ...
    const [itemBinAssignments, setItemBinAssignments] = useState<Record<string, string>>({});
        ...
        const loadInitialData = async () => {
            ...
            if (receipt && !isNewReceipt) {
                try {
                    ...
                    if (fullReceiptData.receivingBin && !fullReceiptData.receivedItems?.every((item: any) => item.receivingBin)) {
                        console.log('🔄 Old receipt detected: adding document-level bin to items');
                        fullReceiptData.receivedItems = (fullReceiptData.receivedItems || []).map((item: any) => ({
                            ...item,
    ...
    useEffect(() => {
        const itemsWithQuantity = (formData.receivedItems || []).filter(item =>
            (item.receivedQuantity || 0) > 0
        );
        const itemsMissingBins = itemsWithQuantity.filter(item =>
    ...
    const saveReceipt = async (status: string = 'draft'): Promise<any> => {
        ...
        if (status === 'completed') {
            const hasItemsWithQuantity = (formData.receivedItems || []).some(item =>
                (item.receivedQuantity || 0) > 0
            );
            ...
            if (!hasItemsWithQuantity) {
                toast({
                    ...
                    description: 'At least one item must have a received quantity greater than 0.',
                    status: 'error',
                    duration: 5000,
                    isClosable: true,
                });
        ...
        if (status === 'completed') {
            const itemsWithoutBins = (formData.receivedItems || []).filter(item =>
                item.receivedQuantity > 0 && !itemBinAssignments[item._key]
            );
    ...
    const validatePricesForCompletion = (): { isValid: boolean; errors: string[] } => {
        const errors: string[] = [];
        ...
        (formData.receivedItems || []).forEach(item => {
            ...
            if (hasQuantity && !hasValidPrice) {
                errors.push(`${item.stockItem.name}: Has quantity (${item.receivedQuantity}) but total price is E 0`);
            }
        });
    ...
    const handleCompleteReceipt = async () => {
        ...
        if (!hasItemsWithQuantity) {
            toast({
                ...
                description: 'You must enter received quantities for at least one item before completing.',
                status: 'error',
                duration: 5000,
                isClosable: true,
            });
        ...
        if (itemsMissingBins.length > 0) {
            ...
            const extraCount = itemsMissingBins.length > 3 ? ` and ${itemsMissingBins.length - 3} more` : '';

            toast({
                title: 'Missing Bin Assignments',
                description: (
                    <VStack align="start" spacing={1}>
                        <Text>{itemsMissingBins.length} item(s) with quantity need bin assignment:</Text>
                        <Text fontSize="sm" fontWeight="medium">{itemNames}{extraCount}</Text>
                        <Text fontSize="xs" color="orange.600">Please assign bins to all items with quantity.</Text>
                    </VStack>
                ),
                status: 'error',
                duration: 7000,
        ...
        const assignedBinIds = [...new Set(Object.values(itemBinAssignments))].filter(id => id);
        const missingBinIds = assignedBinIds.filter(binId =>
            !availableBins.some(bin => bin._id === binId)
        );

        if (missingBinIds.length > 0) {
            toast({
                title: 'Invalid Bin Assignments',
                description: 'Some assigned bins are no longer available. Please re-assign items to valid bins.',
                status: 'error',
                duration: 5000,
                isClosable: true,
            });
        ...
        try {
            ...
            toast({
                ...
                description: 'All validations passed! Please upload evidence files to complete the receipt.',
                status: 'success',
                duration: 3000,
                isClosable: true,
            });
    ...
    const handleFinalizeReceipt = async (attachmentIds: string[]) => {
        ...
        try {
            ...
            const saveResponse = await fetch(`/api/goods-receipts/${receiptIdToUse}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!saveResponse.ok) {
                ...
                throw new Error(errorData.error || `Failed to save receipt with bin assignments`);
            }
            ...
            if (!completeResponse.ok) {
                ...
                if (errorData.error?.includes('missing receiving bins')) {
                    ...
                    if (itemsMissingBins.length > 0) {
                        throw new Error(`Backend validation: ${itemsMissingBins.length} items still missing bin assignments`);
                    } else {
                        // Something else is wrong with the data
                        throw new Error('Backend validation failed. Please check all items have valid bin assignments.');
                    }
                }
            ...
            if (error instanceof Error) {
                if (error.message.includes('missing bin assignments')) {
                    errorMessage = 'Some items are still missing bin assignments. Please assign bins to all items with quantity.';
                ...
                <CardBody>
                    <VStack spacing={4} align="stretch">
                        ...
                        <Text fontSize="sm" color={secondaryTextColor}>
                            Select bins and assign items to distribute goods across multiple storage locations
                        </Text>

                        {/* Bin Selection Row */}
                        <Box p={4} bg={tableHeaderBg} borderRadius="md" borderWidth="1px" borderColor={borderColor}>
                            <Text fontWeight="medium" mb={3}>Select Available Bins:</Text>
                            <SimpleGrid
                                    ...
                                    return (
                                        ...
                                        >
                                            <Card
                                                ...
                                                onClick={isCompletedReceipt ? undefined : () => handleBinToggle(bin._id)}
                                                _hover={{ borderColor: isCompletedReceipt ? undefined : 'brand.300' }}
                                                    ...
                                                    <VStack spacing={2} align="stretch" h="100%">
                                                        ...
                                                        <HStack justify="space-between" spacing={2}>
                                                            <HStack spacing={2} minW="0" flex={1}>
                                                                {isCompletedReceipt ? (
                                                                    <Icon
                                                                        ...
                                                                        color={isBinSelected ? 'green.500' : 'gray.400'}
                                                                        boxSize={4}
                                                                        flexShrink={0}
                                                                    />
                                                                ) : (
                                                            ...
                                                            <Badge
                                                                ...
                                                                size="sm"
                                                                variant={isCompletedReceipt ? "subtle" : "solid"}
                                                                flexShrink={0}
                                                            >
                                                                {bin.binType === 'storage' ? 'Storage' :
                                                                    bin.binType === 'dispatch' ? 'Dispatch' :
                                                                        bin.binType === 'receiving' ? 'Receiving' : bin.binType}
                                                            </Badge>
                                                        </HStack>
                            ...
                            {formData.status === 'completed' && (
                                <Text fontSize="sm" color={secondaryTextColor} mt={3} fontStyle="italic">
                        ...
                        {selectedBinIds.length > 0 && (
                            <Box mt={4}>
                                <Text fontWeight="medium" mb={3}>Assign Items to Bins:</Text>
                                ...
                                <TableContainer>
                                    <Table variant="simple" size="sm">
                                        <Thead>
                                            <Tr bg={tableHeaderBg}>
                                                {selectedBinIds.map(binId => (
                                                    <Th key={binId} color={secondaryTextColor} borderColor={borderColor} textAlign="center">
                                                        {getBinName(binId)}
                                                    </Th>
                                                ))}
                                                ...
                                                <Th color={secondaryTextColor} borderColor={borderColor} textAlign="center">
                                                    Status
                                                </Th>
                                            </Tr>
                                        </Thead>
                                        <Tbody>
                                            {(formData.receivedItems || []).map(item => {
                                                const hasQuantity = (item.receivedQuantity || 0) > 0;
                                                const hasBinAssignment = !!itemBinAssignments[item._key];
                                                const needsBin = hasQuantity && !hasBinAssignment;
                                                const needsPrice = hasQuantity && (item.totalPrice || 0) <= 0;
                                                ...
                                                return (
                                                    <Tr
                                                        ...
                                                        borderLeftColor={needsBin ? warningBorder : 'transparent'}
                                                    >

                                                        {selectedBinIds.map(binId => (
                                                            <Td key={binId} borderColor={borderColor} textAlign="center">
                                                                <Radio
                                                                    isChecked={itemBinAssignments[item._key] === binId}
                                                                    onChange={() => handleItemBinAssignment(item._key, binId)}
                                                                    colorScheme="brand"
                                                        ...
                                                        <Td isNumeric borderColor={borderColor}>
                                                            {item.orderedQuantity || 0} {item.stockItem.unitOfMeasure}
                                                        </Td>
                                                        <Td borderColor={borderColor}>
                                                            <VStack align="start" spacing={1}>
                                                                <Input
                                                                    ...
                                                                    isDisabled={formData.status === 'completed'}
                                                                    bg={inputBg}
                                                                    borderColor={item.receivedQuantity === 0 ? warningBg : needsBin ? warningBorder : borderColor}
                                                                    placeholder="0"
                                                                    _focus={{ borderColor: item.receivedQuantity === 0 ? 'orange.500' : needsBin ? 'orange.500' : 'brand.500' }}
                                                                />
                                                        ...
                                                        <Td borderColor={borderColor}>
                                                            <VStack align="start" spacing={1}>
                                                                <Input
                                                                    ...
                                                                    isDisabled={formData.status === 'completed'}
                                                                    bg={inputBg}
                                                                    borderColor={needsPrice ? 'red.300' : borderColor}
                                                                    _focus={{ borderColor: needsPrice ? 'red.500' : 'brand.500' }}
                                                                    placeholder="0.00"
                                                                />
                                                                ...
                                                                {item.totalPrice as number > 0 && item.receivedQuantity > 0 && (
                                                                    <Text fontSize="xs" color="green.600">
                                                                        Unit: E {((item.totalPrice as number) / item.receivedQuantity).toFixed(2)}
                                                                    </Text>
                                                                )}
                                                            </VStack>
                                                        </Td>
                                                        <Td borderColor={borderColor}>
                                                            ...
                                                            {needsPrice && (
                                                                <Text fontSize="xs" color="red.500" fontWeight="medium" mt={1}>
                                                                    Set price
                                                                </Text>
                                                            )}
                                                        </Td>
                                                        <Td borderColor={borderColor} textAlign="center">
                                                            {needsBin ? (
                                                                <Tooltip label="This item has quantity but no bin assigned">
                                                                    <Badge colorScheme="orange" variant="solid" fontSize="xs" cursor="help">
                                                                        Needs Bin
                                                                    </Badge>
                                                                </Tooltip>
                                                            ) : hasBinAssignment ? (
                                                                <Tooltip label={`Assigned to ${getBinName(itemBinAssignments[item._key])}`}>
                                                                    <Badge colorScheme="green" variant="subtle" fontSize="xs" cursor="help">
                                                                        ✓ Assigned
                                                                    </Badge>
                                                                </Tooltip>
                                                            ) : !hasQuantity ? (
                                ...
                                {/* Summary */}
                                <Box mt={4} p={4} bg={tableHeaderBg} borderRadius="md" borderWidth="1px" borderColor={borderColor}>
                                    <HStack justify="space-between" mb={3}>
                                        <Text fontWeight="medium">Assignment Summary:</Text>
                                        <Badge
                                            colorScheme={itemsWithoutBins.length > 0 ? "orange" : "green"}
                                            variant={itemsWithoutBins.length > 0 ? "solid" : "subtle"}
                                        >
                                            {itemsWithoutBins.length > 0 ? `${itemsWithoutBins.length} items need bins` : 'All items assigned'}
                                        </Badge>
                                    </HStack>
                                    <SimpleGrid columns={{ base: 1, md: 2 }} spacing={3}>
                                        {selectedBinIds.map(binId => {
                                            const bin = availableBins.find(b => b._id === binId);
                                            const assignedItems = (formData.receivedItems || []).filter(
                                                item => itemBinAssignments[item._key] === binId
                                            );
                                            const totalQuantity = assignedItems.reduce((sum, item) => sum + item.receivedQuantity, 0);
                                            const totalValue = assignedItems.reduce((sum, item) => sum + (item.totalPrice || 0), 0);
                                                ...
                                                >
                                                    <HStack>
                                                        ...
                                                        <VStack align="start" spacing={0}>
                                                            <Text fontSize="sm" fontWeight="medium">{bin?.name}</Text>
                                                            <Text fontSize="xs" color={secondaryTextColor}>{bin?.binType}</Text>
                                                        </VStack>
                                                    </HStack>
                                    ...
                                    {itemsWithoutBins.length > 0 && (
                                        <Box mt={3} p={2} bg={warningBg} borderRadius="md" borderWidth="1px" borderColor={warningBorder}>
                                            <Text fontSize="xs" color="orange.700">
                                                <strong>Action required:</strong> Items highlighted in orange have received quantity but no bin assignment.
                                                Please assign a bin to each item before completing the receipt.
                                            </Text>
                                        </Box>
                                    )}
                                </Box>
                ...
                <ModalContent bg={modalBg} maxW={{ base: '100%', md: '1200px' }} mx="auto" my={{ base: 0, md: 8 }} borderRadius={{ base: 'none', md: 'xl' }} boxShadow="xl" height="90vh">
                            ...
                            <VStack spacing={4} align="stretch" color={primaryTextColor}>
                                ...
                                >
                                    ...
                                    <FormControl>
                                        ...
                                        <Input value={formData.purchaseOrder?.site?.name || 'N/A'} isReadOnly bg={inputBg} borderColor={borderColor} />
                                    </FormControl>
                                </HStack>
                                ...
                                {availableBins.length > 0 ? (
                                    ...
                                    <Card bg={warningBg} borderWidth="1px" borderColor={warningBorder} mt={4}>
                                        <CardBody>
                                            <HStack>
                                                <Icon as={FiAlertCircle} color="orange.500" />
                                                <VStack align="start" spacing={1}>
                                                    <Text fontWeight="medium" color="orange.700">No Bins Available</Text>
                                                    <Text fontSize="sm" color="orange.600">
                                                        No bins found for site: {formData.purchaseOrder?.site?.name || 'Unknown Site'}
                                                    </Text>
                                                </VStack>
                                            </HStack>
                                        </CardBody>
                                                            ...
                                                            <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4} w="100%">
                                                                {formData.attachments
                                                                    ...
                                                                    .map((attachment) => {
                                                                        ...
                                                                        const isJPG = ['jpg', 'jpeg'].includes(fileExtension);
                                                                        const isImageFile = isPNG || isJPG || ['png', 'jpg', 'jpeg', 'gif', 'webp'].includes(fileExtension);
                                                                                ...
                                                                                {isImageFile && fileUrl ? (
                                                                                    <Box position="relative" height="200px">
                                                                                        <Image
                                                                                            ...
                                                                                            loading="lazy"
                                                                                            onError={(e) => {
                                                                                                console.warn('Image failed to load, showing fallback:', fileUrl);
                                                                                                // Hide the image and show fallback
                                                                                                e.currentTarget.style.display = 'none';
                                                                                                const fallbackElement = e.currentTarget.parentElement?.querySelector('.image-fallback');
                                                                                                if (fallbackElement) {
                                                                                ...
                                                                                <Box p={3}>
                                                                                    ...
                                                                                    <Text fontSize="xs" color={secondaryTextColor} mt={1}>
                                                                                        {attachment.description && !isInvoiceAttachment(attachment.description)
                                                                                            ...
                                                                                            : (isImageFile ? 'Photo' : `${fileExtension.toUpperCase()} File`)
                                                                                        }
                                                                                    </Text>
                                                                                    {fileUrl && isImageFile && (
                    ...
                    >
                        ...
                        <Button
                            ...
                            flexShrink={0}
                        >
                            <Text as="span" display={{ base: "none", md: "inline" }}>Track Invoice</Text>
                            <Text as="span" display={{ base: "inline", md: "none" }}>Invoice</Text>
                        </Button>
                        ...
                        {isEditable && (
                            <>
                                ...
                                <Button
                                    ...
                                    flexShrink={0}
                                >
                                    <Text as="span" display={{ base: "none", sm: "inline" }}>Save Draft</Text>
                                    <Text as="span" display={{ base: "inline", sm: "none" }}>Draft</Text>
                                </Button>
                                ...
                                <Button
                                    ...
                                    title={
                                        itemsWithoutBins.length > 0
                                            ...
                                            : (formData.receivedItems || []).some(item =>
                                                item.receivedQuantity > 0 && (item.totalPrice || 0) <= 0
                                            )
