...
import { getArchivedPurchaseOrderById } from "@/lib/archiveQueries";
...
export async function GET(
  ...
  try {
    ...
    let poDetails = await client.fetch(query, { id });

    if (!poDetails) {
      const archived = await getArchivedPurchaseOrderById(id);
      if (!archived) {
        return NextResponse.json(
