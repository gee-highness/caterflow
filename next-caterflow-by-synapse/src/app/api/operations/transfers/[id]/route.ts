// src/app/api/operations/transfers/[id]/route.ts
...
import { getArchivedTransferById } from "@/lib/archiveQueries";
...
export async function GET(
  ...
  try {
    ...
    let transfer = await client.fetch(query, { id });

    if (!transfer) {
      const archived = await getArchivedTransferById(id);
      if (!archived) {
  ...
  try {
    ...
    if (body.status === "pending-approval") {
      if (
        ...
        body.transferredItems.length === 0
      ) {
        return NextResponse.json(
