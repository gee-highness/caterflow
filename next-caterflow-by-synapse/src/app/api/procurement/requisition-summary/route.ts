...
export async function GET(request: Request) {
  try {
    ...
    const supplierId = searchParams.get("supplierId"); // ✅ ADD THIS
    ...
    if (supplierId && supplierId !== "all") {
      baseQuery += ` && count(orderedItems[ supplier._ref == $supplierId ]) > 0`;
      queryParams.supplierId = supplierId;
    }

    // ✅ ADD category filter
    if (category && category !== "all") {
      baseQuery += ` && count(orderedItems[ stockItem->category->title == $category ]) > 0`;
...
`;
    ...
    const summary = await processRequisitionSummary(
      purchaseOrders,
      userSiteInfo,
    );
    ...
    const res = NextResponse.json(
      {
        error: "Failed to fetch requisition summary",
 ...
 */
async function processRequisitionSummary(
  purchaseOrders: any[],
  userSiteInfo: any,
) {
  const items: any[] = [];
  ...
  allCategories.forEach((category: any) => {
    if (
      category.siteMatchPatterns &&
      Array.isArray(category.siteMatchPatterns)
    ) {
  ...
  const itemsBySite = Array.from(sites.values()).map((site) => {
    ...
    const siteSupplierIds = new Set(
      siteItems.map((item) => item.supplierId).filter(Boolean),
    );
    ...
    const itemsByCategory = Array.from(siteCategoryMap.entries()).map(
      ([category, categoryItems]) => {
        ...
        const itemsBySubCategory = Array.from(subCategoryMap.entries()).map(
          ([subCategory, subCategoryItems]) => {
            const subCategoryTotal = subCategoryItems.reduce(
              (sum, item) => sum + item.amount,
              0,
            );
            return {
    ...
    const siteSuppliers = Array.from(siteSupplierIds)
      .map((id) => {
  ...
  const itemsByCategory = Array.from(categories).map((category) => {
    ...
    const categoryDetails = allCategories.find(
      (c: { title: string }) => c.title === category,
    );
    ...
    const itemsBySite = Array.from(siteMap.entries()).map(
      ([siteKey, siteItems]) => {
  ...
  const itemsBySupplier = Array.from(suppliers.values()).map((supplier) => {
    const supplierItems = items.filter(
      (item) => item.supplierId === supplier._id,
    );
    const supplierTotal = supplierItems.reduce(
      (sum, item) => sum + item.amount,
      0,
    );
    const supplierSiteIds = new Set(
      supplierItems.map((item) => item.siteId).filter(Boolean),
    );
    ...
    return {
      ...
      performance: {
        ...
        purchaseOrderCount: new Set(supplierItems.map((item) => item.poNumber))
          .size,
        firstOrderDate:
        ...
        lastOrderDate:
          supplierItems.length > 0
            ...
            : undefined,
        averageOrderValue:
          supplierItems.length > 0 ? supplierTotal / supplierItems.length : 0,
        itemsPerPO:
          supplierItems.length /
          (new Set(supplierItems.map((item) => item.poNumber)).size || 1),
        siteCount: supplierSiteIds.size,
        sites: Array.from(supplierSiteIds)
          .map((id) => sites.get(id))
          .filter(Boolean) as any[],
      },
  ...
  const stats = {
    ...
    averageSupplierPerSite:
      itemsBySite.length > 0 ? uniqueSuppliers / itemsBySite.length : 0,
  };
