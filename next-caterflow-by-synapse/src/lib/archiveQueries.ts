...
export async function getArchivedTransactionsForItem(
    ...
    const receipts = await db.collection(COLLECTIONS.GOODS_RECEIPTS).find({
        'receivedItems': {
            $elemMatch: {
    ...
    for (const r of receipts) {
        for (const item of (r.receivedItems || [])) {
            if (item.stockItem?._id === stockItemId && item.receivingBin?._id === binId) {
                transactions.push({
                    type: 'receipt',
                    date: r.receiptDate,
    ...
    const dispatches = await db.collection(COLLECTIONS.DISPATCH_LOGS).find({
        'dispatchedItems': {
            $elemMatch: {
                'stockItem._id': stockItemId,
                'sourceBin._id': binId,
    ...
    for (const d of dispatches) {
        for (const item of (d.dispatchedItems || [])) {
            if (item.stockItem?._id === stockItemId && item.sourceBin?._id === binId) {
    ...
    const transfersOut = await db.collection(COLLECTIONS.INTERNAL_TRANSFERS).find({
        'fromBin._id': binId,
        'transferredItems': {
    ...
    const transfersIn = await db.collection(COLLECTIONS.INTERNAL_TRANSFERS).find({
        'toBin._id': binId,
        'transferredItems': {
            $elemMatch: { 'stockItem._id': stockItemId }
        }
    ...
    const counts = await db.collection(COLLECTIONS.INVENTORY_COUNTS).find({
        'bin._id': binId,
        'countedItems': {
            $elemMatch: { 'stockItem._id': stockItemId }
        }
